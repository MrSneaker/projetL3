...
Graph::Graph(vector<pair<double, double>> data1_, string graphName_, int xMin_, int yMin_, int xMax_, int yMax_)
{
    data1 = data1_;
    graphName = graphName_;
    xMin = xMin_;
...
int Graph::createGraph()
{
    ...
    if (gp.good())
    {
        ...
        gp << "plot" << gp.file1d(data1) << "with lines title '" + graphName + "1" + "'," << gp.file1d(data2) << "with lines title '" + graphName + "2" + "',"
           << gp.file1d(data3) << "with lines title '" + graphName + "3" + "'," << std::endl;
        ret = 0;
    }
    else
        ret = -1;
